...
namespace CSharpLua
{
    public sealed partial class LuaSyntaxNodeTransformer
    {
        ...
        private LuaArrayTypeAdapterExpressionSyntax BuildArrayTypeFromInitializer(InitializerExpressionSyntax initializer)
        {
            ...
            var symbol = _semanticModel.GetTypeInfo(firstExpression).Type;
        ...
        {
            var typeName = !_generator.IsSealed(type) ? GetTypeName(baseType) : new LuaMemberAccessExpressionSyntax(LuaIdentifierNameSyntax.This, LuaIdentifierNameSyntax.Base);
        ...
        {
            IMethodSymbol ctorSymbol = _semanticModel.GetDeclaredSymbol(node);
            _methodInfos.Push(new MethodInfo(ctorSymbol));
            ...
            if (node.Initializer != null)
            {
                var symbol = (IMethodSymbol)_semanticModel.GetSymbolInfo(node.Initializer).Symbol;
            ...
            if (isStatic)
            {
                CurrentType.SetStaticCtor(function);
            }
            else
            {
                CurrentType.AddCtor(function, node.ParameterList.Parameters.Count == 0);
            }
        ...
        {
            IMethodSymbol symbol = (IMethodSymbol)_semanticModel.GetSymbolInfo(body.Parent).Symbol;
            _methodInfos.Push(new MethodInfo(symbol));
            ...
            {
                var type = (INamedTypeSymbol)_semanticModel.GetTypeInfo(body.Parent).ConvertedType;
        ...
        public override LuaSyntaxNode VisitThrowStatement(ThrowStatementSyntax node)
        {
            ...
            {
                var curTryFunction = (LuaTryAdapterExpressionSyntax)CurrentFunction;
        ...
        private BaseVisitType CheckBaseVisitType<T>(MemberAccessExpressionSyntax parent, T symbol, Func<T, ISymbol> overriddenFunc) where T : ISymbol
        {
            if (symbol.IsOverridable())
            {
                ...
                if (_generator.IsSealed(curTypeSymbol))
                {
        ...
        public override LuaSyntaxNode VisitBaseExpression(BaseExpressionSyntax node)
        {
            ...
            var symbol = _semanticModel.GetSymbolInfo(parent).Symbol;
        ...
        {
            bool isEmpty = _functions.Count == 0;
            if (isEmpty)
            {
                LuaFunctionExpressionSyntax function = new LuaFunctionExpressionSyntax();
                PushFunction(function);
            ...
            if (isRoot)
            {
                _conditionalTemporaries.Push(GetTempIdentifier(node.Expression));
            }

            var temp = _conditionalTemporaries.Peek();
            var expression = (LuaExpressionSyntax)node.Expression.Accept(this);
            if (isRoot)
            {
                CurrentBlock.Statements.Add(new LuaLocalVariableDeclaratorSyntax(temp, expression));
            }
            else
            {
                CurrentBlock.Statements.Add(new LuaExpressionStatementSyntax(new LuaAssignmentExpressionSyntax(temp, expression)));
            }
        ...
        public override LuaSyntaxNode VisitMemberBindingExpression(MemberBindingExpressionSyntax node)
        {
            ...
            if (node.Name.Identifier.ValueText == "Invoke")
            {
                ...
                {
                    return _conditionalTemporaries.Peek();
                }
            }

            var nameExpression = (LuaExpressionSyntax)node.Name.Accept(this);
            return new LuaMemberAccessExpressionSyntax(_conditionalTemporaries.Peek(), nameExpression, symbol.Kind == SymbolKind.Method);
        }
        ...
        public override LuaSyntaxNode VisitElementBindingExpression(ElementBindingExpressionSyntax node)
        {
            ...
            var memberAccess = new LuaMemberAccessExpressionSyntax(_conditionalTemporaries.Peek(), new LuaIdentifierNameSyntax(LuaSyntaxNode.Tokens.Get), true);
            LuaInvocationExpressionSyntax invocation = new LuaInvocationExpressionSyntax(memberAccess);
            invocation.ArgumentList.Arguments.AddRange(argumentList.Arguments);
            return invocation;
        }
        ...
        public override LuaSyntaxNode VisitElementAccessExpression(ElementAccessExpressionSyntax node)
        {
            ...
            var symbol = (IPropertySymbol)_semanticModel.GetSymbolInfo(node).Symbol;
        ...
        private void BuildOperatorMethodDeclaration(BaseMethodDeclarationSyntax node)
        {
            ...
            LuaBlockSyntax block = (LuaBlockSyntax)node.Body.Accept(this);
            function.AddStatements(block.Statements);
            CurrentType.AddMethod(name, function, isPrivate, symbol.IsStaticLazy(), comments);
        ...
        public override LuaSyntaxNode VisitDeclarationExpression(DeclarationExpressionSyntax node)
        {
            ...
            {      //out var 
                ...
                _blocks.Peek().Statements.Add(new LuaLocalVariableDeclaratorSyntax(name));
                return name;
            }
            else
        ...
        public override LuaSyntaxNode VisitIsPatternExpression(IsPatternExpressionSyntax node)
        {
            ...
            var expression = (LuaExpressionSyntax)node.Expression.Accept(this);
            _blocks.Peek().Statements.Add(new LuaLocalVariableDeclaratorSyntax(name, expression));

            var leftType = _semanticModel.GetTypeInfo(node.Expression).Type;
