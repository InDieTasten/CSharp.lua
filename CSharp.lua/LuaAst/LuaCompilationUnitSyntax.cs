...
namespace CSharpLua.LuaAst
{
    public sealed class LuaCompilationUnitSyntax : LuaSyntaxNode
    {
        ...
        private bool _isImportLinq;
        private int _typeDeclarationCount;
        private List<UsingDeclare> _usingDeclares = new List<UsingDeclare>();
        ...
        public bool IsEmpty
        {
            get
            {
                return _typeDeclarationCount == 0;
            }
        }
        ...
        {
            _typeDeclarationCount++;
        }

        internal void AddImport(string prefix, string newPrefix, bool isFromCode)
        {
            if (!_usingDeclares.Exists(i => i.Prefix == prefix))
            {
                _usingDeclares.Add(new UsingDeclare()
                {
                    Prefix = prefix,
                    NewPrefix = newPrefix,
        ...
        {
            var imports = _usingDeclares.Where(i => !i.IsFromCode).ToList();
            if (imports.Count > 0)
            {
            ...
            var usingDeclares = _usingDeclares.Where(i => i.IsFromCode).ToList();
