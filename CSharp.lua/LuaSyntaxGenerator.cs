    ...
    public sealed class LuaSyntaxGenerator
    {
        public sealed class SettingInfo
        {
            ...
            public int Indent
            {
                ...
                set
                {
                    if (value > 0 && _indent != value)
                    {
                        _indent = value;
                        IndentString = new string(' ', _indent);
                    }
                }
            }
        }

        private const string _luaFileExtension = ".lua";
        private static readonly Encoding Encoding = new UTF8Encoding(false);

        private CSharpCompilation _compilation;
        public XmlMetaProvider XmlMetaProvider { get; }
        public SettingInfo Setting { get; set; }
        private HashSet<string> _exportEnums = new HashSet<string>();
        private bool _isExportAttributesAll;
        private HashSet<string> _exportAttributes;
        private List<LuaEnumDeclarationSyntax> _enumDeclarations = new List<LuaEnumDeclarationSyntax>();
        private Dictionary<INamedTypeSymbol, List<PartialTypeDeclaration>> _partialTypes = new Dictionary<INamedTypeSymbol, List<PartialTypeDeclaration>>();
        private IMethodSymbol _mainEntryPoint;
        ...
        public LuaSyntaxGenerator(IEnumerable<SyntaxTree> syntaxTrees, IEnumerable<MetadataReference> references, CSharpCompilationOptions options, IEnumerable<string> metas, SettingInfo setting, string[] attributes, string baseFolder = "")
        {
            ...
            if (attributes != null)
            {
                ...
                {
                    _isExportAttributesAll = true;
                }
                else
                {
                    _exportAttributes = new HashSet<string>(attributes);
                }
            }
            DoPretreatment();
        }
        ...
        private IEnumerable<LuaCompilationUnitSyntax> Create()
        {
            ...
            foreach (SyntaxTree syntaxTree in _compilation.SyntaxTrees)
            {
                ...
                LuaSyntaxNodeTransformer transfor = new LuaSyntaxNodeTransformer(this, semanticModel);
        ...
        private string GetOutFilePath(string inFilePath, string output_, out string module)
        {
            ...
            path = path.Replace('.', '_');
            string outPath = Path.Combine(output_, path + _luaFileExtension);
        ...
        internal bool IsCheckedOverflow
        {
            get
            {
                return _compilation.Options.CheckOverflow;
            }
        }

        internal bool IsEnumExport(string enumTypeSymbol)
        {
            return _exportEnums.Contains(enumTypeSymbol);
        }

        internal void AddExportEnum(string enumTypeSymbol)
        {
            _exportEnums.Add(enumTypeSymbol);
        }

        internal void AddEnumDeclaration(LuaEnumDeclarationSyntax enumDeclaration)
        {
            _enumDeclarations.Add(enumDeclaration);
        }

        internal bool IsExportAttribute(INamedTypeSymbol attributeTypeSymbol)
        {
            if (_isExportAttributesAll)
            {
                return true;
            }
            else
            {
                if (_exportAttributes != null && _exportAttributes.Count > 0)
                {
                    if (_exportAttributes.Contains(attributeTypeSymbol.ToString()))
                    {
                        return true;
                    }
                }
        ...
        {
            foreach (var enumDeclaration in _enumDeclarations)
            {
        ...
        {
            var list = _partialTypes.GetOrDefault(typeSymbol);
            if (list == null)
            {
                list = new List<PartialTypeDeclaration>();
                _partialTypes.Add(typeSymbol, list);
            }
        ...
        {
            while (_partialTypes.Count > 0)
            {
                var types = _partialTypes.Values.ToArray();
                _partialTypes.Clear();
                foreach (var typeDeclarations in types)
                {
                    PartialTypeDeclaration major = typeDeclarations.Min();
                    LuaSyntaxNodeTransformer transfor = new LuaSyntaxNodeTransformer(this, null);
        ...
        private List<INamedTypeSymbol> GetExportTypes()
        {
            ...
            if (_types.Count > 0)
            {
                _types.Sort((x, y) => x.ToString().CompareTo(y.ToString()));

                List<List<INamedTypeSymbol>> typesList = new List<List<INamedTypeSymbol>>() { _types };
                while (true)
                {
                    ...
                    foreach (var type in lastTypes)
                    {
                        ...
                        var attributes = _typeDeclarationAttributes.GetOrDefault(type);
                        if (attributes != null)
                        {
        ...
        private void ExportManifestFile(List<string> modules, string outFolder)
        {
            ...
            const string dirInitCode = "dir = (dir and #dir > 0) and (dir .. '.') or \"\"";
            const string require = "require";
            const string loadCode = "local load = function(module) return require(dir .. module) end";
            const string load = "load";
            const string init = "System.init";
            const string manifestFile = "manifest.lua";
            ...
            if (modules.Count > 0)
            {
                ...
                if (types.Count > 0)
                {
                    ...
                    functionExpression.AddParameter(new LuaIdentifierNameSyntax(dir));
                    functionExpression.AddStatement(new LuaIdentifierNameSyntax(dirInitCode));

                    LuaIdentifierNameSyntax requireIdentifier = new LuaIdentifierNameSyntax(require);
                    functionExpression.AddStatement(new LuaLocalVariableDeclaratorSyntax(requireIdentifier, requireIdentifier));

                    functionExpression.AddStatement(new LuaIdentifierNameSyntax(loadCode));
                    functionExpression.AddStatement(LuaBlankLinesStatement.One);

                    LuaIdentifierNameSyntax loadIdentifier = new LuaIdentifierNameSyntax(load);
                    ...
                    LuaInvocationExpressionSyntax initInvocation = new LuaInvocationExpressionSyntax(new LuaIdentifierNameSyntax(init), typeTable);
                    ...
                    string outFile = Path.Combine(outFolder, manifestFile);
                    Write(luaCompilationUnit, outFile);
                }
            }
        }
        ...
        private void FillManifestInitConf(LuaInvocationExpressionSyntax invocation)
        {
            ...
            if (_mainEntryPoint != null)
            {
                LuaIdentifierNameSyntax methodName = new LuaIdentifierNameSyntax(_mainEntryPoint.Name);
                var methodTypeName = GetTypeName(_mainEntryPoint.ContainingType);
        ...
        private Dictionary<ISymbol, LuaSymbolNameSyntax> _memberNames = new Dictionary<ISymbol, LuaSymbolNameSyntax>();
        private Dictionary<INamedTypeSymbol, HashSet<string>> _typeNamesUsed = new Dictionary<INamedTypeSymbol, HashSet<string>>();
        private HashSet<ISymbol> _refactorNames = new HashSet<ISymbol>();
        private Dictionary<INamedTypeSymbol, HashSet<INamedTypeSymbol>> _extends = new Dictionary<INamedTypeSymbol, HashSet<INamedTypeSymbol>>();
        private List<INamedTypeSymbol> _types = new List<INamedTypeSymbol>();
        private Dictionary<INamedTypeSymbol, HashSet<INamedTypeSymbol>> _typeDeclarationAttributes = new Dictionary<INamedTypeSymbol, HashSet<INamedTypeSymbol>>();
        private Dictionary<ISymbol, LuaSymbolNameSyntax> _propertyOrEventInnerFieldNames = new Dictionary<ISymbol, LuaSymbolNameSyntax>();
        private Dictionary<ISymbol, string> _memberIllegalNames = new Dictionary<ISymbol, string>();
            ...
            foreach (ISymbol member in sameNameMembers)
            {
                ...
                else
                {
                    if (!_memberNames.ContainsKey(member))
                    {
                        ...
                        _memberNames.Add(member, new LuaSymbolNameSyntax(identifierName));
                    }
                }
                if (index > 0)
                {
        ...
        internal LuaIdentifierNameSyntax AddInnerName(ISymbol symbol)
        {
            ...
            _propertyOrEventInnerFieldNames.Add(symbol, symbolName);
            return symbolName;
        }
        ...
        {
            return _typeNamesUsed.TryAdd(type, newName);
        }
        ...
        private void CheckRefactorNames()
        {
            ...
            foreach (ISymbol symbol in _refactorNames)
            {
        ...
        private void RefactorCurTypeSymbol(ISymbol symbol, HashSet<ISymbol> alreadyRefactorSymbols)
        {
            ...
            var childrens = _extends.GetOrDefault(typeSymbol);
        ...
        private bool RefactorInterfaceSymbol(ISymbol symbol, HashSet<ISymbol> alreadyRefactorSymbols)
        {
            if (symbol.IsFromCode())
            {
                ...
                var childrens = _extends[typeSymbol];
        ...
        {
            var childrens = _extends.GetOrDefault(curType);
            if (childrens != null)
            {
                foreach (INamedTypeSymbol children in childrens)
                {
        ...
        {
            _memberNames[symbol].Update(newName);
        ...
        private string GetRefactorName(INamedTypeSymbol typeSymbol, IEnumerable<INamedTypeSymbol> childrens, ISymbol symbol)
        {
            ...
            int index;
            if (_memberIllegalNames.TryGetValue(symbol, out string originalName))
            {
                index = 0;
            }
            else
        ...
        {
            var set = _typeNamesUsed.GetOrDefault(typeSymbol);
        ...
        {
            foreach (var innerName in _propertyOrEventInnerFieldNames)
            {
        ...
        #endregion
        private Dictionary<ISymbol, HashSet<ISymbol>> _implicitInterfaceImplementations = new Dictionary<ISymbol, HashSet<ISymbol>>();
        private Dictionary<INamedTypeSymbol, Dictionary<ISymbol, ISymbol>> _implicitInterfaceTypes = new Dictionary<INamedTypeSymbol, Dictionary<ISymbol, ISymbol>>();
        private Dictionary<IPropertySymbol, bool> _isFieldProperties = new Dictionary<IPropertySymbol, bool>();
        private HashSet<INamedTypeSymbol> _typesOfExtendSelf = new HashSet<INamedTypeSymbol>();
        ...
        private sealed class PretreatmentChecker : CSharpSyntaxWalker
        {
            ...
            public PretreatmentChecker(LuaSyntaxGenerator generator)
            {
                ...
                foreach (SyntaxTree syntaxTree in generator._compilation.SyntaxTrees)
                {
            ...
            {
                var semanticModel_ = generator_._compilation.GetSemanticModel(node.SyntaxTree);
            ...
            private void CheckImplicitInterfaceImplementation(INamedTypeSymbol type)
            {
                if (type.TypeKind == TypeKind.Class && !type.IsStatic)
                {
                    ...
                    {
                        generator_._typesOfExtendSelf.Add(type);
                    }
                }
            }
            ...
            private void RefactorTypeName(INamedTypeSymbol type, string name, int index)
            {
                ...
                generator_._typeRefactorNames.Add(type, newName);
            }
            ...
            private void RefactorNamespaceName(INamespaceSymbol[] all, INamespaceSymbol symbol, string name, int index)
            {
                ...
                generator_._namespaceRefactorNames.Add(symbol, newName);
            }
        }
        ...
        {
            bool success = _implicitInterfaceImplementations.TryAdd(implementationMember, interfaceMember);
            if (success)
            {
                var containingType = implementationMember.ContainingType;
                var mapps = _implicitInterfaceTypes.GetOrDefault(containingType);
                if (mapps == null)
                {
                    mapps = new Dictionary<ISymbol, ISymbol>();
                    _implicitInterfaceTypes.Add(containingType, mapps);
                }
        ...
        {
            var mapps = _implicitInterfaceTypes.GetOrDefault(typeSymbol);
            return mapps?.GetOrDefault(interfaceMember);
        }

        private bool IsImplicitInterfaceImplementation(ISymbol symbol)
        {
            return _implicitInterfaceImplementations.ContainsKey(symbol);
        }

        internal bool IsPropertyField(IPropertySymbol symbol)
        {
            if (!_isFieldProperties.TryGetValue(symbol, out bool isAutoField))
            {
                ...
                _isFieldProperties.Add(symbol, isAutoField);
            }
            return isAutoField;
        }
        ...
        private IEnumerable<ISymbol> AllInterfaceImplementations(ISymbol symbol)
        {
            ...
            var implicitImplementations = _implicitInterfaceImplementations.GetOrDefault(symbol);
        ...
        {
            return typeSymbol.HasStaticConstructor() || _typesOfExtendSelf.Contains(typeSymbol);
        }

        internal bool IsExtendExists(INamedTypeSymbol typeSymbol)
        {
            var set = _extends.GetOrDefault(typeSymbol);
        ...
        private Dictionary<INamespaceSymbol, string> _namespaceRefactorNames = new Dictionary<INamespaceSymbol, string>();
        private Dictionary<INamedTypeSymbol, string> _typeRefactorNames = new Dictionary<INamedTypeSymbol, string>();

        private string GetTypeRefactorName(INamedTypeSymbol symbol)
        {
            return _typeRefactorNames.GetOrDefault(symbol);
        }
        ...
        internal LuaExpressionSyntax GetTypeName(ISymbol symbol, LuaSyntaxNodeTransformer transfor = null)
        {
        ...
        private List<LuaExpressionSyntax> GetTypeArguments(INamedTypeSymbol typeSymbol, LuaSyntaxNodeTransformer transfor)
        {
        ...
        private void FillExternalTypeArgument(List<LuaExpressionSyntax> typeArguments, INamedTypeSymbol typeSymbol, LuaSyntaxNodeTransformer transfor)
        {
        ...
        private void FillTypeArguments(List<LuaExpressionSyntax> typeArguments, INamedTypeSymbol typeSymbol, LuaSyntaxNodeTransformer transfor)
        {
        ...
        private string GetNamespaceMapName(INamespaceSymbol symbol, string original)
        {
            if (symbol.IsFromCode())
            {
                var names = symbol.GetAllNamespaces().Select(i => _namespaceRefactorNames.GetOrDefault(i, i.Name));
        ...
        internal LuaIdentifierNameSyntax GetTypeShortName(ISymbol symbol, LuaSyntaxNodeTransformer transfor = null)
        {
